...
export default function Navbar({ menu, setMenu }) {
  ...
  const visibleLinks = ["About", "Contact"];
  const dropdownLinks = [
          ...
          <div className="hidden md:flex space-x-6">
            {visibleLinks.map((link) => (
              <button
                ...
                className={`${menu === link
                    ? "bg-[#FF6F61] text-white px-4 py-1 rounded-sm"
            ...
            >
              <button
                ...
                className={`flex items-center ${menu === "Categories"
                    ? "bg-[#FF6F61] text-white px-4 py-1 rounded-sm"
                    : "text-gray-700 hover:text-[#FF6F61]"
                  } text-lg font-medium`}
              >
                ...
                {dropdownOpen && (
                  ...
                  >
                    {dropdownLinks.map((link) => (
                      <button
                        key={link.value}
                        onClick={() => setMenu(link.value)} // ✅ sets correct state
                        className={`block w-full text-left px-2 py-1 rounded ${menu === link.value
...
<div className="flex-shrink-0 text-2xl font-bold text-gray-900 mx-auto md:mx-0 z-50 relative">
  <button
    onClick={() => {
      setMenu("All");       // Reset state to All
  ...
  >
    ...
    />
      ...
      {/* ✅ Mobile Dropdown (mirrors desktop behavior) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  {dropdownLinks.map((link) => (
                    <button
                      ...
                      className={`text-gray-600 hover:text-[#FF6F61] transition ${menu === link.value ? "font-bold text-[#FF6F61]" : ""
                        }`}
                    >
                      {link.label}
                    </button>
